...
6. 无 EPC 编码的商品直接使用 DEFAULT_CAR_NUM（001）兜底小车号。
...
| ⑧ | 无 EPC 编码的商品，使用 `DEFAULT_CAR_NUM`（001）兜底小车号 |
...
| 编号 | 事项 | 现状 |
|------|------|------|
| 1 | 下发 PLC 的标识已确认为 EPC 编码（即商品编码 inco） | 客户已确认 |
| 2 | UI 查询键已确认为 EPC（商品编码） | 客户已确认 |
...
| 12 | UI 查询 | 操作员 → WCS | 按EPC（商品编码）查物件 |