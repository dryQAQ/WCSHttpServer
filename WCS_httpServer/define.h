...
// ──── ★ 2026-09-18 新增：波次 SKU→格口 映射留痕（log/WAVE_MAP/wave_map.log）────
//   背景：现场要"每次从任务下发解析出的 item 中，把 SKU 对应的格口留一份记录"。
//   成本实测（本机：20000 行同步落盘 335ms ≈ 0.017ms/行）：
//     4,114 件波次 ≈ 4,007 行 ≈ 0.07s；50,000 件波次 ≈ 32,000 行 ≈ 0.54s；
//     且写入点在 ParseWorker 解析线程（低优先级），**不占主线程/UI、不持 DB 锁**。
//   闸门：SKU 数超过下面这个值 → 只写"摘要"，避免超大波次把日志写成几十万行。
//     现场若要强制全量留痕（自行承担日志量）：设环境变量 WCS_WAVE_MAP_FULL=1。
#define WAVE_MAP_LOG_MAX_SKU     5000   // SKU 种类数 ≤ 此值逐 SKU 落行；超过只落摘要（可调）
